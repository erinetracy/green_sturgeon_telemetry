...
# probability at alternative route arrays (Georgiana Slough and
# Steamboat/Sutter Sloughs), following Perry et al. (2018).
#
# BIOLOGICAL RATIONALE:
# Higher flows may reduce detection probability by increasing acoustic
# noise or by increasing fish transit speed past receiver arrays.
# We test this on alternative route arrays only (p_geo, p_ss) where
# flow effects on detection are most plausible given variable channel
# geometry. Mainstem Sacramento arrays are in constrained channels
# where detection geometry is less likely to vary with flow.
#
# MODEL STRUCTURE:
# Identical to model 08 plus:
#   logit(p_geo[i])  = alpha_p_geo  + beta_p_geo  * flow_occ2[i]
#   logit(p_ss[i])   = alpha_p_ss   + beta_p_ss   * flow_occ3[i]
#
# INTERPRETATION:
# If beta_p_geo and beta_p_ss credible intervals include zero:
#   -> flow does not meaningfully affect detection at these arrays
#   -> model 08 results are robust
# If either clearly excludes zero:
#   -> update model 08 to include flow-dependent detection
#==============================================================================
...
load("C:/Users/eetracy/Desktop/R_directory/ST_telemetry/gs_multistate/gs_mcmc_flow_08_run1.RData")
...
#==============================================================================
# SECTION 3: REBUILD FLOW COVARIATES
# Identical to model 08 — flow_occ2, flow_occ3, flow_30day
#==============================================================================
...
# --- Extract vectors ---
flow_occ2_09  <- fish_flow_09$flow_occ2_std;  flow_occ2_09[is.na(flow_occ2_09)]   <- 0
flow_occ3_09  <- fish_flow_09$flow_occ3_std;  flow_occ3_09[is.na(flow_occ3_09)]   <- 0
flow_30day_09 <- fish_flow_09$flow_30day_std; flow_30day_09[is.na(flow_30day_09)] <- 0
...
# Identical to model 08 plus flow-dependent detection at Geo and SS arrays
#==============================================================================
...
nimCode_09 <- nimbleCode({
  ...
  # Flow-dependent detection at alternative route arrays
  # logit(p_geo[i]) = alpha_p_geo + beta_p_geo * flow_occ2[i]
  # logit(p_ss[i])  = alpha_p_ss  + beta_p_ss  * flow_occ3[i]
  alpha_p_geo ~ dnorm(0, sd = 1.5)
  beta_p_geo  ~ dnorm(0, sd = 1.0)
  ...
  for(i in 1:nfish){
    ...
    # Flow-dependent detection at alternative route arrays
    p_geo_i[i]  <- ilogit(alpha_p_geo + beta_p_geo * flow_occ2[i])
    p_ss_i[i]   <- ilogit(alpha_p_ss  + beta_p_ss  * flow_occ3[i])
    
  }
  
  #--- OBSERVATION ARRAY ---
  # Note: p_geo and p_ss are now individual-level so observation
  # matrix must be built inside the fish loop
  
  # Fixed detection arrays — occasions where detection is constant
  ...
  # Occasion 2: p_geo is fish-specific — handled in fish loop below
  # Placeholder row 2 for Geo state — will be overridden per fish
  p_arr[1, 1:6, 2] <- c(p_sac2, 0, 0, 0, 0, (1-p_sac2))
  p_arr[2, 1:6, 2] <- c(0, 0.5, 0, 0, 0, 0.5)  # placeholder — overridden
  p_arr[3, 1:6, 2] <- c(0, 0, 0, 0, 0, 1)
  p_arr[4, 1:6, 2] <- c(0, 0, 0, 1, 0, 0)
  p_arr[5, 1:6, 2] <- c(0, 0, 0, 0, 1, 0)
  p_arr[6, 1:6, 2] <- c(0, 0, 0, 0, 0, 1)
  
  # Occasion 3: p_ss is fish-specific — handled in fish loop below
  p_arr[1, 1:6, 3] <- c(p_sac3, 0, 0, 0, 0, (1-p_sac3))
  p_arr[2, 1:6, 3] <- c(0, 0, 0, 0, 0, 1)
  p_arr[3, 1:6, 3] <- c(0, 0, 0.5, 0, 0, 0.5)  # placeholder — overridden
  ...
  for(i in 1:nfish){
    ...
    # Build fish-specific observation array
    # Only occ2 Geo row and occ3 SS row differ from fixed p_arr
    ...
    # Occasion 2 — fish-specific p_geo_i
    p_arr_i[i, 1, 1:6, 2] <- p_arr[1, 1:6, 2]
    p_arr_i[i, 2, 1:6, 2] <- c(0, p_geo_i[i], 0, 0, 0, (1-p_geo_i[i]))
    ...
    # Occasion 3 — fish-specific p_ss_i
    ...
    # Occasions 4-7 — same for all fish
    ...
    # Transition matrices — identical to model 08
    ...
    # Likelihood — now uses fish-specific observation array
    ch_mat[i, 1:7] ~ dDHMMo(
...
inits_09 <- list(
  ...
  alpha_p_geo = 1.21,  beta_p_geo = 0.0,  # ilogit(1.21) ~ 0.77 = baseline p_geo
  alpha_p_ss  = 0.79,  beta_p_ss  = 0.0   # ilogit(0.79) ~ 0.69 = baseline p_ss
)
...
inits_fn_09 <- function(){
  list(
    ...
    alpha_p_geo = rnorm(1, 1.21,  0.3), beta_p_geo = rnorm(1, 0, 0.2),
    alpha_p_ss  = rnorm(1, 0.79,  0.3), beta_p_ss  = rnorm(1, 0, 0.2)
  )
}
...
MCMCtrace(mcmc_out_09,
          params   = c("alpha_p_geo", "beta_p_geo",
                       "alpha_p_ss",  "beta_p_ss",
                       "alpha_geo", "beta_geo",
                       "alpha_ss",  "beta_ss",
                       "alpha_fail", "beta_fail"),

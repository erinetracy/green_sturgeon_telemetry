...
nimCode_flow <- nimbleCode({
  ...
  #--- LIKELIHOOD ---
  # Fish-specific transition matrices because psi_geo[i] and psi_ss[i] vary
  # by individual based on flow. Each fish gets its own dDHMMo call.
  # This is the main computational difference from the baseline model.
  # Transitions where routing doesn't apply (occ3->4 through occ6->7) are
  # identical across fish and use the same survival and failure parameters.
  
  for(i in 1:nfish){
    
    # Transition 1->2: Benicia to Rio Vista (Georgiana routing decision)
    # psi_geo[i] is fish-specific based on flow_occ2[i]
    tr_arr_i[1, 1:6, 1] <- c(S_sac*(1-psi_geo[i])*(1-phi_fail),
                             S_sac*psi_geo[i]*(1-phi_fail),
                             0, (1-S_sac), S_sac*phi_fail, 0)
    tr_arr_i[2, 1:6, 1] <- c(0, 0, 0, 1, 0, 0)
    tr_arr_i[3, 1:6, 1] <- c(0, 0, 0, 1, 0, 0)
    tr_arr_i[4, 1:6, 1] <- c(0, 0, 0, 1, 0, 0)
    tr_arr_i[5, 1:6, 1] <- c(0, 0, 0, 0, 1, 0)
    tr_arr_i[6, 1:6, 1] <- c(0, 0, 0, 0, 0, 1)
    
    # Transition 2->3: Rio Vista to SR_MOUTH/Steamboat (SS routing decision)
    # psi_ss[i] is fish-specific based on flow_occ3[i]
    tr_arr_i[1, 1:6, 2] <- c(S_sac*(1-psi_ss[i])*(1-phi_fail),
                             0,
                             S_sac*psi_ss[i]*(1-phi_fail),
                             (1-S_sac), S_sac*phi_fail, 0)
    tr_arr_i[2, 1:6, 2] <- c(0, 1, 0, 0, 0, 0)
    tr_arr_i[3, 1:6, 2] <- c(0, 0, 0, 1, 0, 0)
    tr_arr_i[4, 1:6, 2] <- c(0, 0, 0, 1, 0, 0)
    tr_arr_i[5, 1:6, 2] <- c(0, 0, 0, 0, 1, 0)
    tr_arr_i[6, 1:6, 2] <- c(0, 0, 0, 0, 0, 1)
    
    # Transition 3->4: SR_MOUTH to SR_BLWSTEAM
    # Geo rejoins Sac, SS passes through (can fail) - same for all fish
    tr_arr_i[1, 1:6, 3] <- c(S_sac*(1-phi_fail), 0, 0,
                             (1-S_sac), S_sac*phi_fail, 0)
    tr_arr_i[2, 1:6, 3] <- c(S_geo*(1-phi_fail), 0, 0,
                             (1-S_geo), S_geo*phi_fail, 0)
    tr_arr_i[3, 1:6, 3] <- c(0, 0, (1-phi_fail), 0, phi_fail, 0)
    tr_arr_i[4, 1:6, 3] <- c(0, 0, 0, 1, 0, 0)
    tr_arr_i[5, 1:6, 3] <- c(0, 0, 0, 0, 1, 0)
    tr_arr_i[6, 1:6, 3] <- c(0, 0, 0, 0, 0, 1)
    
    # Transition 4->5: SR_BLWSTEAM to SR_KK345R (SS rejoins Sacramento)
    tr_arr_i[1, 1:6, 4] <- c(S_sac*(1-phi_fail), 0, 0,
                             (1-S_sac), S_sac*phi_fail, 0)
    tr_arr_i[2, 1:6, 4] <- c(0, 0, 0, 1, 0, 0)
    tr_arr_i[3, 1:6, 4] <- c(S_ss*(1-phi_fail), 0, 0,
                             (1-S_ss), S_ss*phi_fail, 0)
    tr_arr_i[4, 1:6, 4] <- c(0, 0, 0, 1, 0, 0)
    tr_arr_i[5, 1:6, 4] <- c(0, 0, 0, 0, 1, 0)
    tr_arr_i[6, 1:6, 4] <- c(0, 0, 0, 0, 0, 1)
    
    # Transition 5->6: SR_FREEPORT to upper Sacramento
    tr_arr_i[1, 1:6, 5] <- c(S_sac*(1-phi_fail), 0, 0,
                             (1-S_sac), S_sac*phi_fail, 0)
    tr_arr_i[2, 1:6, 5] <- c(0, 0, 0, 1, 0, 0)
    tr_arr_i[3, 1:6, 5] <- c(0, 0, 0, 1, 0, 0)
    tr_arr_i[4, 1:6, 5] <- c(0, 0, 0, 1, 0, 0)
    tr_arr_i[5, 1:6, 5] <- c(0, 0, 0, 0, 1, 0)
    tr_arr_i[6, 1:6, 5] <- c(0, 0, 0, 0, 0, 1)
    
    # Transition 6->7: upper Sacramento to spawning ground
    tr_arr_i[1, 1:6, 6] <- c(lambda*(1-phi_fail), 0, 0,
                             (1-lambda), lambda*phi_fail, 0)
    tr_arr_i[2, 1:6, 6] <- c(0, 0, 0, 1, 0, 0)
    tr_arr_i[3, 1:6, 6] <- c(0, 0, 0, 1, 0, 0)
    tr_arr_i[4, 1:6, 6] <- c(0, 0, 0, 1, 0, 0)
    tr_arr_i[5, 1:6, 6] <- c(0, 0, 0, 0, 1, 0)
    tr_arr_i[6, 1:6, 6] <- c(0, 0, 0, 0, 0, 1)
    
    # Likelihood for fish i using fish-specific transition matrix
    ch_mat[i, 1:7] ~ dDHMMo(
      init         = c(1, 0, 0, 0, 0, 0)[1:6],
      probObs      = p_arr[1:6, 1:6, 1:7],
      probTrans    = tr_arr_i[1:6, 1:6, 1:6],
      len          = 7,
      checkRowSums = 0
    )
